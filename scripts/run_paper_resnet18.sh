...
for dataset in "${DATASET_ARGS[@]}"; do
  ...
  case "${WANDB}" in
    1|true|TRUE|yes|YES)
      command+=(
        ...
        --wandb_group "${dataset_tag}-resnet18-paper30"
        --wandb_tags "${dataset}" resnet18 paper30 full
      )
  ...
  echo "Running ${dataset} with ResNet-18, methods=${METHODS}, seeds=${SEEDS}"
  "${command[@]}"
done
...
echo "Summary CSV: outputs/paper30_r18_results.csv"
