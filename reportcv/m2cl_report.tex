...
$i$ at layer $l$ ($\|\bm{u}_i^{(l)}\|=1$), $N_b$ the batch size, and $N_c$ the
number of samples of class $c$ in the batch. The paper defines the following
class-conditional probability for each layer $l$:
\begin{equation}
    p^{(l)}(c)
    \;=\;
    \frac{
        \displaystyle\sum_{i=1}^{N_c}
        \sum_{\substack{j=i+1\\y_j=c}}^{N_c}
        e^{\,{\bm{u}_i^{(l)}}^\top \bm{u}_j^{(l)}/\tau}
    }{
        \displaystyle\sum_{k=1}^{N_b}
        \sum_{m=k+1}^{N_b}
        e^{\,{\bm{u}_k^{(l)}}^\top \bm{u}_m^{(l)}/\tau}
    },
    \label{eq:p_layer}
\end{equation}
where $\tau$ is the temperature. The numerator sums pairwise similarities over
same-class pairs; the denominator sums over \emph{all} pairs in the batch and is
therefore the same for every class (computed once per layer per batch). Taking the
product over all $C$ classes and then the negative log gives the layer loss:
\begin{equation}
    \mathcal{L}^{(l)}
    \;=\;
    -\log\!\prod_{c=1}^{C} p^{(l)}(c)
    \;=\;
...
\begin{equation}
    ...
    \;+\;
    \alpha \sum_{l=1}^{M} \mathcal{L}^{(l)},
    \label{eq:total_loss}
\end{equation}
where $\mathcal{L}_{\mathrm{CE}}$ is cross-entropy, $M$ is the number of
extraction layers, and $\alpha$ controls the contrastive weight. Default values
follow the paper: $\alpha=0.01$ and $\tau=1.0$.
...
The contrastive-loss module first computes cross-entropy on the final logits. If the method is \base{}, the contrastive weight is set to zero. If the method is \method{}, the module computes a layer score $S_l$ as the log-sum of same-class pairwise similarities normalized by the full-batch pairwise sum. Because $S_l$ is a \emph{score} (higher is better) while $\mathcal{L}^{(l)}$ from Eq.~(\ref{eq:L_layer}) is a \emph{loss} (lower is better), we have $S_l = -\mathcal{L}^{(l)}$, and the combined loss becomes:
\[
    ...
    \mathcal{L}_{\mathrm{CE}}
    +
    \alpha \sum_l \mathcal{L}^{(l)},
\]
which is exactly Eq.~(\ref{eq:total_loss}). In the code, activations are L2-normalized and pairwise inner products are used as cosine similarities.
...
\textit{Unbiased Look at Dataset Bias},
