...
The original paper evaluates on PACS, VLCS, Office-Home, and NICO. The current repository contains dataset loaders for all four datasets, but the report focuses mainly on PACS, VLCS, and Office-Home because those are the datasets used in the project tables and slides. NICO is mentioned when discussing qualitative saliency examples and future extensions, but it is not treated as a completed quantitative reproduction.
...
\begin{enumerate}[leftmargin=1.6em]
    ...
    \item It separates real experimental outputs from one-seed estimates and synthetic placeholders, so the reader can judge the strength of the empirical evidence correctly.
...
\codepath{train.py} & Builds datasets, splits source domains, trains, validates, checkpoints, and writes metrics. \\
\codepath{data/} & Contains dataset loaders for PACS, VLCS, Office-Home, and NICO. \\
...
The repository expects datasets to be placed under a common data root and provides \codepath{check\_data.py} to validate folder layout before long runs. A single PACS run with \method{} can be launched as:
\[
    \texttt{\small python train.py --dataset pacs --test\_domain photo --method m2cl}
\]
VLCS and Office-Home use the same pattern with different dataset and domain arguments.

For full paper-style experiments, \codepath{run\_experiments.py} and the scripts under \codepath{scripts/} iterate over datasets, target domains, methods, backbones, and seeds. The intended score for each benchmark is the average over held-out target domains and random seeds. The training script saves the checkpoint with the best source validation accuracy, reloads it after training, evaluates the held-out target domain, and writes metric JSON files for later aggregation.
...
\subsection{Result Provenance and Reliability}

The appendix tables distinguish three types of values. Values without suffix are completed JSON results averaged over three seeds. Values marked with \ones{} are real JSON outputs from one seed and should be interpreted as preliminary estimates. Values marked with \syn{} are synthetic placeholders used for presentation planning when the full run was not available. This distinction is important for academic honesty and for interpreting the strength of the conclusions.

\begin{table}[H]
\centering
\caption{Reliability interpretation for result suffixes used in the appendix.}
\label{tab:result_reliability}
\small
\begin{adjustbox}{max width=\textwidth}
\begin{tabular}{lll}
\toprule
Suffix & Meaning & How it should be used \\
\midrule
none & Real result averaged over three seeds & Suitable for main quantitative claims. \\
\ones{} & Real result from one seed & Useful for trend checking, but variance is unknown. \\
\syn{} & Synthetic placeholder & Should not be treated as completed experimental evidence. \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}

Because some \base{} and \method{} entries are placeholders, the safest conclusion is qualitative rather than absolute: the implemented method follows the expected direction from the paper, and the available tables are consistent with \method{} improving over \base{} and strong baselines on average. A final research-grade reproduction should rerun all placeholder cells with the same seeds, hardware profile, and checkpoint-selection rule.
...
\begin{itemize}[leftmargin=1.4em]
    \item The paper reports complete experiments on four datasets, including NICO. This report focuses quantitatively on PACS, VLCS, and Office-Home.
    \item The paper presents final benchmark tables. This report explicitly marks which project-table entries are completed results, one-seed estimates, or placeholders.
...
There are three main threats to validity in the current project. First, incomplete runs limit the strength of quantitative conclusions. Placeholder values are useful for formatting and presentation planning, but they cannot replace real experiments. Second, one-seed results may not represent the true average because DG performance can vary across random seeds. Third, implementation choices such as data augmentation, optimizer defaults, and source validation splits may differ from the original paper or DomainBed defaults.
...
The second lesson is about reproducibility. A convincing DG experiment requires careful source-target separation, source-only checkpoint selection, multiple seeds, and clear result provenance. The most important future improvement is therefore completing all placeholder cells with real multi-seed experiments, then reporting both mean and standard deviation. Additional useful extensions include NICO quantitative results, memory/time profiling, and failure-case saliency analysis.
