...
\begin{document}
	...
	\begin{frame}{Introduction to Domain Generalization}
		...
		\begin{alertblock}{The Strict Constraint vs. Domain Adaptation}
			\begin{itemize}
				\item \textbf{Domain Adaptation:} Has access to unlabeled target domain data during training.
				\item \textbf{Domain Generalization:} \alert{Zero access} to target-domain data or target labels during the optimization cycle.
			\end{itemize}
		\end{alertblock}
	\end{frame}
...
\end{frame}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.55\linewidth]{photos/dataset_cv1.png}
	...
	\begin{frame}{Dataset Table : PACS and VLCS (ResNet-18)}
\begin{table}[htbp]
\centering
	...
	\begin{columns}[T]
		...
		\begin{column}{0.5\textwidth}
			\begin{block}{Key Observations}
				\begin{itemize}
					\item $M^2$-CL achieves the best overall balance across all domains.
					\item Largest improvement: \textbf{Sketch} (+7.9 pp vs. ERM), indicating robustness to severe style shifts.
					\item Gains on \textbf{Art} suggest better cross-domain feature alignment.
				\end{itemize}
			\end{block}
		\end{column}
	\end{columns}
...
\end{alertblock}
	...
	\begin{frame}{Table: Ablation Study on PACS and VLCS}
\begin{table}[htbp]
\centering
...
\begin{frame}{Feature Space Visualization: ERM vs $M^2$-CL}
	\begin{figure}
		\centering
		\includegraphics[width=0.65\linewidth]{photos/tsne_comparison.png}
	\end{figure}
	\vspace{-0.3cm}
	\begin{block}{}
...
\begin{frame}{Visualizing Robustness via Saliency Maps}
	...
	\begin{figure}
		\centering
		\includegraphics[width=0.8\linewidth]{photos/Pacs.png}
