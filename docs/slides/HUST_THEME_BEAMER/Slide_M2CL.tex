			...
			\begin{column}{0.50\textwidth}
				\begin{alertblock}{What $M^2$-CL Contributes}
					\begin{itemize}
						...
						\item \textbf{Spatial Dropout} in each block --- prevents style shortcuts from leaking.
						\item \textbf{Layer-wise contrastive loss} --- enforces same-class alignment across domains at \emph{all} scales simultaneously.
					\end{itemize}
				\end{alertblock}
			\end{column}
		\end{columns}
		...
		\begin{columns}[T]
			\begin{column}{0.45\textwidth}
				\begin{block}{The ERM Trap}
					\begin{itemize}
						\item Minimizes training loss by exploiting \alert{superficial shortcuts} (textures, backgrounds) rather than object geometry.
						\item These cues \textbf{vanish or invert} under domain shift.
					\end{itemize}
				\end{block}
				\begin{alertblock}{This Motivates $M^2$-CL}
					\begin{itemize}
						\item Need to \textbf{force the model} to rely on structural, domain-invariant features.
						\item Solution: align same-class representations across domains at \emph{multiple scales}.
					\end{itemize}
				\end{alertblock}
			\end{column}
	...
	\begin{columns}[T]
		\begin{column}{0.42\textwidth}
			...
			\begin{alertblock}{Spatial Dropout}
				\small Drops \textbf{entire feature maps} ($H \times W$ channels). Forces the network to rely on alternative feature maps, \textbf{blocking style shortcuts} at the source.
			\end{alertblock}
		\end{column}
    ...
    \begin{columns}[t]
        ...
        \begin{column}{0.44\textwidth}
            ...
            \begin{exampleblock}{Matrix Parallelization}
                \begin{itemize}
                    \item Prevents memory bottlenecks.
...
\end{frame}
		...
		\begin{columns}[T]
			...
			\begin{column}{0.50\textwidth}
				\begin{alertblock}{Architecture \& Loss}
					\begin{itemize}
						\item \textbf{$r = 4$:} Ablation (Table 3) shows best accuracy/memory trade-off; $r{=}2$ overfits, $r{=}6$ under-compresses.
						\item \textbf{$\alpha = 0.01$:} Contrastive term guides without dominating CE loss; larger $\alpha$ collapses to 64\% PACS.
						\item \textbf{$\tau = 1.0$:} Robust default; sensitivity analysis (Appendix) shows stable performance across $\tau \in [0.8, 1.4]$.
					\end{itemize}
				\end{alertblock}
			\end{column}
		\end{columns}
	...
	\begin{frame}{Benchmark Datasets}
		...
		\begin{figure}
			\centering
			\includegraphics[width=0.60\linewidth]{photos/dataset_cv1.png}
		\end{figure}
	\end{frame}
	...
	\begin{columns}[T]
		...
		\begin{column}{0.45\textwidth}
			...
			\begin{alertblock}{$M^2$-CL (Right)}
				\small The layer-wise contrastive loss \textbf{collapses intra-class domain scatter}: all domains for the same class cluster tightly, while inter-class boundaries stay sharp.\\[4pt]
				\textbf{Insight:} The network has learned truly \emph{domain-invariant} representations, directly validating the contrastive objective.
			\end{alertblock}
		\end{column}
	\end{columns}
\end{frame}
	...
	\begin{columns}[T]
		...
		\begin{column}{0.45\textwidth}
			...
			\begin{alertblock}{$M^2$-CL}
				\small Saliency concentrates on \textbf{object contours and edges}, consistent across Photo, Art, Cartoon, and Sketch domains.\\[4pt]
				\textbf{Insight:} The contrastive loss forces the network to anchor decisions on \emph{structural geometry}, not style --- the intended inductive bias.
			\end{alertblock}
		\end{column}
	\end{columns}
\end{frame}
