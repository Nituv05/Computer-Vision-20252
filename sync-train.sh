#!/usr/bin/env bash
# =============================================================================
# sync-train.sh  —  Đẩy code LAPTOP -> SERVER, chọn 1 H100, train song song, báo xong.
#
...
#     cd <thư-mục-project> && bash sync-train.sh
#     # --- hoặc tách từng bước nếu muốn debug ---
#     bash sync-train.sh setup                       # đẩy code + cài Miniconda + env + deps
#     bash sync-train.sh data                        # tải dataset (pacs/vlcs/office_home) về server
#     bash sync-train.sh run                         # chỉ đẩy code + bật train
#     ~/sync-train.sh log                            # xem log realtime (lần chạy mới nhất)
#     ~/sync-train.sh logs                           # liệt kê TẤT CẢ log các lần train trước
#     ~/sync-train.sh attach                         # vào màn hình job (thoát: Ctrl-b d)
#     ~/sync-train.sh status                         # job còn chạy? GPU thế nào?
#     ~/sync-train.sh pull                            # kéo logs+outputs từ server VỀ LAPTOP (backup)
#     ~/sync-train.sh watch                          # đợi train xong: vừa chờ vừa kéo log về (notify)
#     ~/sync-train.sh stop                           # dừng job
#     # W&B: mở .env.wandb trên laptop, dán WANDB_API_KEY rồi chạy sync-train.sh
# =============================================================================
...
WANDB_ENV_FILE=".env.wandb"   # File secret local. Bị gitignore, nhưng sync-train.sh sẽ đẩy riêng lên server.

ENTRY="SEEDS=0 BATCH_SIZE=32 bash scripts/run_paper_resnet18.sh"
...
RESUME_FLAG="--resume"        # Cờ mà train.py NHẬN để nạp checkpoint. << train.py phải hỗ trợ cờ này.
...
require_wandb_env() {
  ...
  if ! wandb_env_ready; then
    ...
    echo "   Nếu muốn train không dùng W&B, đặt USE_WANDB=0 trong sync-train.sh."
    exit 1
  fi
}
...
cd "$DEST"
python download_data.py --data_root "$DATA_ROOT" --datasets $DATASETS
...
case "$ACTION" in
  ...
  setup)  do_setup; echo "   Tải dataset: ~/sync-train.sh data   |   Hoặc làm hết 1 lệnh: ~/sync-train.sh all"; exit 0 ;;
  data)   do_data; exit 0 ;;
...
if ssh "$HOST" "tmux has-session -t $SESSION 2>/dev/null"; then
  echo "⚠  Session '$SESSION' đang chạy. Dừng nó trước: ~/sync-train.sh stop"; exit 1
fi
...
ln -sfn "${DATASET_ARR[0]}_\$TS.log" logs/latest.log    # 'log' bám dataset đầu; xem cái khác: ~/sync-train.sh logs
echo RUNNING > STATUS
...
python summarize_results.py --metrics_dir outputs/paper30_r18 \
...
echo "   Xem log  : ~/sync-train.sh logs   (mỗi dataset 1 file <dataset>_<time>.log)"
echo "   Backup   : ~/sync-train.sh pull   (kéo logs+outputs nhẹ, không checkpoint, về $LOCAL_BACKUP)"
echo "   Đợi xong : ~/sync-train.sh watch  (vừa đợi vừa tự backup về laptop)"
echo "   Tình trạng: ~/sync-train.sh status   |   Dừng: ~/sync-train.sh stop"
